...
// Multiclass helper: explain the per-class aggregated score (same algebra as -S)
// for a specific class index in [0, nr_class).
double svm_predict_and_explain_values_for_score_class(
    const struct svm_model *model, const union svm_data x,
    double* dec_values, double **explanation, int mode, int score_class);
double svm_predict_and_singlebaseexplain_values_for_score_class(
    const struct svm_model *model, const union svm_data x,
    double* dec_values, double *explanation, int score_class);
// END NEW
