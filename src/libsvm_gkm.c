...
// Memory-efficient variants that accumulate directly into an aggregated explanation
static void kmertree_dfs_withexplanation_weighted(const KmerTree *tree,
    const int last_seqid, const int depth, const int curr_node_index,
    const BaseMismatchCountForExplanation *curr_matching_bases,
    const int curr_num_matching_bases, double **explanation,
    const double *eff_weight)
{
    int i, j, k;
    int bid;
    ...
    if (depth == tree->depth - 1) {
        ...
        for (bid = 1; bid <= MAX_ALPHABET_SIZE; bid++) {
            ...
            for (j = 0; j < curr_num_matching_bases; j++) {
                ...
                const int currbase_mmcnt = curr_matching_bases[j].mmcnt;
                const int seqpos = curr_matching_bases[j].seqpos;
                const uint8_t *hist = curr_matching_bases[j].base_lmer_match_history;
                ...
                if (currbase == bid) {
                    ...
                    for (i = 0; i < leaf_cnt; i++) {
                        int svid = data[i].seqid;
                        if (svid >= last_seqid) continue;
                        double wsv = eff_weight[svid];
                        if (wsv == 0.0) continue;
                        double weighted_alpha = alpha * (data[i].wt * currbase_wt) * wsv;
                        int total_matches = 0;
                        for (k = 0; k < L; k++) {
                            uint8_t base_then = *(curr_matching_bases[j].bid - ((L - 1) - k));
                            if (hist[k] == 1 || (k == L - 1)) {
                                explanation[seqpos + k][base_then - 1] += weighted_alpha;
                                total_matches += 1;
                            }
                        }
                        assert(total_matches == (L - currbase_mmcnt));
        ...
        for (bid = 1; bid <= MAX_ALPHABET_SIZE; bid++) {
            daughter_node_index++;
            if (tree->node[daughter_node_index] <= 0) continue;
            ...
            for (j = 0; j < curr_num_matching_bases; j++) {
                ...
                int currbase_mmcnt = curr_matching_bases[j].mmcnt;
                int currbase_seqpos = curr_matching_bases[j].seqpos;
                uint8_t *hist = curr_matching_bases[j].base_lmer_match_history;
                ...
                if (*currbase_ptr == bid) {
                    ...
                    next_matching_bases[next_num_matching_bases].seqpos = currbase_seqpos;
                    hist[depth] = 1;
                    next_matching_bases[next_num_matching_bases].base_lmer_match_history = hist;
                    next_num_matching_bases++;
                } else if (currbase_mmcnt < d) {
                    next_matching_bases[next_num_matching_bases].bid = currbase_ptr + 1;
                    next_matching_bases[next_num_matching_bases].wt = curr_matching_bases[j].wt;
            ...
            if (next_num_matching_bases > 0) {
                kmertree_dfs_withexplanation_weighted(tree, last_seqid, depth + 1,
                    daughter_node_index, next_matching_bases, next_num_matching_bases,
                    explanation, eff_weight);
            }
        }
    }
}

static void kmertree_dfs_withhypexplanation_weighted(const KmerTree *tree,
    const int last_seqid, const int depth, const int curr_node_index,
    const BaseMismatchCountForExplanation *curr_matching_bases,
    const int curr_num_matching_bases, double **explanation, int *tree_lmer,
    const double *eff_weight, uint8_t one_mismatch_deeper, uint8_t perturbation_eff)
{
    int i, j, k, h;
    int bid;
    ...
    if (depth == tree->depth - 1) {
        ...
        for (bid = 1; bid <= MAX_ALPHABET_SIZE; bid++) {
            ...
            for (j = 0; j < curr_num_matching_bases; j++) {
                ...
                for (i = 0; i < leaf_cnt; i++) {
                    ...
                    double weighted_alpha = to_weight * alpha * wsv;
                    double weighted_beta = to_weight * beta * wsv;
                    double weighted_gamma = to_weight * gamma * wsv;
                    double weighted_kappa = to_weight * kappa * wsv;
                    int total_matches = 0;

                    for (k = 0; k < L; k++) {
                        uint8_t base_then = *(curr_matching_bases[j].bid - ((L - 1) - k));
                        int tree_base = tree_lmer[k];
                        if (base_then == tree_base) {
                            assert(hist[k] == 1);
                        } else {
                            assert(hist[k] == 0);
                        }
                        for (h = 1; h <= MAX_ALPHABET_SIZE; h++) {
                            if (base_then == tree_base) {
                                if (h == tree_base) {
                                    explanation[seqpos + k][h - 1] += weighted_alpha;
                                    total_matches += 1;
                                } else {
                                    explanation[seqpos + k][h - 1] += weighted_gamma;
                                }
                            } else {
                                if (h == tree_base) {
                                    explanation[seqpos + k][h - 1] += weighted_kappa;
                                } else {
                                    explanation[seqpos + k][h - 1] += weighted_beta;
                                }
                            }
                        }
                    }
        ...
        for (bid = 1; bid <= MAX_ALPHABET_SIZE; bid++) {
            ...
            if (next_num_matching_bases > 0) {
                kmertree_dfs_withhypexplanation_weighted(tree, last_seqid, depth + 1,
                    daughter_node_index, next_matching_bases, next_num_matching_bases,
                    explanation, tree_lmer, eff_weight, one_mismatch_deeper, perturbation_eff);
            }
        }
    }
}
...
// Instead of producing a per-support-vector explanation tensor and summing it later,
// this directly accumulates into the final aggregated explanation[seqlen][4].
// This keeps memory usage roughly O(nSV + seqlen) instead of O(nSV * seqlen).
double* gkmexplainkernel_kernelfunc_batch_sv_weighted(const gkm_data *da, double *res, double **explanation, int mode, const double *sv_weights)
{
    if (g_sv_kmertree == NULL) {
        clog_error(CLOG(LOGGER_ID), "in gkmexplainkernel_kernelfunc_batch_sv_weighted, kmertree for SVs has not been initialised. call gkmkernel_init_sv() first.");
        return NULL;
    }

    const int nsv = g_sv_num;
    const int seqlen = da->seqlen;
    const int L = g_param->L;

    // Pass 1: compute per-SV kernel values (raw), then normalise and apply RBF if needed.
    for (int j = 0; j < nsv; j++) res[j] = 0;
    gkmkernel_kernelfunc_batch_ptr(da, g_sv_kmertree, 0, nsv, res);

    double *eff_weight = (double *)malloc(sizeof(double) * (size_t)nsv);
    if (!eff_weight) {
        clog_error(CLOG(LOGGER_ID), "malloc failed for eff_weight");
        exit(1);
    }

    const double da_sqnorm = da->sqnorm;
    const int is_rbf = (g_param->kernel_type == EST_TRUNC_RBF || g_param->kernel_type == EST_TRUNC_PW_RBF);
    const double ref = is_rbf ? exp(g_param->gamma * (-1)) : 0.0;

    for (int j = 0; j < nsv; j++) {
        const double denom = da_sqnorm * g_sv_svm_data[j].d->sqnorm;
        const double per_sv_total = (denom != 0.0) ? (res[j] / denom) : 0.0;

        double scale = 1.0;
        if (is_rbf) {
            const double rbf = exp(g_param->gamma * (per_sv_total - 1));
            const double diff_from_ref = rbf - ref;
            res[j] = rbf;
            if (per_sv_total > 0) {
                scale = diff_from_ref / per_sv_total;
            } else {
                scale = 0.0;
            }
        } else {
        ...
        const double w = sv_weights ? sv_weights[j] : 1.0;
        eff_weight[j] = (denom != 0.0) ? (w * scale / denom) : 0.0;
    }

    // Pass 2: DFS over the SV kmertree, accumulating explanation using eff_weight.
    BaseMismatchCountForExplanation matching_bases[MAX_SEQ_LENGTH];
    int num_matching_bases = seqlen - L + 1;
    if (num_matching_bases < 0) num_matching_bases = 0;
    if (num_matching_bases > MAX_SEQ_LENGTH) num_matching_bases = MAX_SEQ_LENGTH;

    for (int i = 0; i < num_matching_bases; i++) {
        matching_bases[i].bid = da->seq + i;
        matching_bases[i].wt = da->wt[i];
        matching_bases[i].mmcnt = 0;
        matching_bases[i].seqpos = i;
        matching_bases[i].base_lmer_match_history = (uint8_t *)malloc(sizeof(uint8_t) * ((size_t)L));
        if (!matching_bases[i].base_lmer_match_history) {
            clog_error(CLOG(LOGGER_ID), "malloc failed for base_lmer_match_history");
            exit(1);
        }
    }
    ...
    switch (mode) {
        case 0:
            kmertree_dfs_withexplanation_weighted(g_sv_kmertree, nsv, 0, 0,
                matching_bases, num_matching_bases, explanation, eff_weight);
            break;
        case 1:
        case 2:
        case 3:
        case 4: {
            int *tree_lmer = (int *)malloc(sizeof(int) * ((size_t)L));
            if (!tree_lmer) {
                clog_error(CLOG(LOGGER_ID), "malloc failed for tree_lmer");
                exit(1);
            }
            uint8_t one_mismatch_deeper = (mode == 2 || mode == 4) ? 1 : 0;
            uint8_t perturbation_eff = (mode == 3 || mode == 4) ? 1 : 0;
            kmertree_dfs_withhypexplanation_weighted(g_sv_kmertree, nsv, 0, 0,
                matching_bases, num_matching_bases, explanation, tree_lmer,
                eff_weight, one_mismatch_deeper, perturbation_eff);
            free(tree_lmer);
            break;
        }
        default:
