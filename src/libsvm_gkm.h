...
// Memory-efficient variant: directly accumulates an aggregated explanation (seqlen x MAX_ALPHABET_SIZE)
// using per-SV weights (length = number of SVs). Produces the same k-values as gkmexplainkernel_kernelfunc_batch_sv.
double* gkmexplainkernel_kernelfunc_batch_sv_weighted(const gkm_data *d, double *res, double **explanation, int mode, const double *sv_weights);
