...
void print_usage_and_exit()
{
    printf(
            "\n"
            "Program: gkmtrain (lsgkm program for SVM model training + probability estimates)\n"
            "Version: "
            LSGKM_VERSION
            "\n\n"
            "Usage: gkmtrain [options] <class1.fa> <class2.fa> [class3.fa ...] <outprefix>\n"
            "\n"
            " train gkm-SVM using libSVM (supports 2+ classes)\n"
            "\n"
            "Arguments:\n"
            " classN.fa: class N sequence file (FASTA format)\n"
...
void read_problem_multi(const char **classfiles, int nclasses);
...
int main(int argc, char** argv)
{
    ...
    param.probability = 0; //probability estimates enabled in this binary
    param.nu = 0.5; //not used
    cross_validation = 0;
    icv = 0;

    int c;
        while ((c = getopt (argc, argv, "t:l:k:d:g:M:H:c:e:w:m:x:i:r:sv:T:RPC:")) != -1) {
		switch (c) {
            case 't':
    ...
    int index = optind;
    int nclasses = (argc - optind) - 1; // last arg is outprefix
    char **classfiles = (char **) malloc(sizeof(char*) * (size_t) nclasses);
    for (int i = 0; i < nclasses; i++) {
        classfiles[i] = argv[index++];
    }
    ...
    for (int i = 0; i < nclasses; i++) {
        clog_info(CLOG(LOGGER_ID), "  class_%d = %s", i+1, classfiles[i]);
    }
    ...
    if (checkpoint_prefix) {
        clog_info(CLOG(LOGGER_ID), "  checkpoint prefix = %s", checkpoint_prefix);
    }
    ...
    if (checkpoint_prefix) {
        svm_set_checkpoint(checkpoint_prefix, 0);
    }
    ...
    read_problem_multi((const char**)classfiles, nclasses);
    ...
    if (checkpoint_prefix) {
        char ckpt_path[2048];
        int n = snprintf(ckpt_path, sizeof(ckpt_path), "%s.train.ckpt", checkpoint_prefix);
        if (n > 0 && n < (int)sizeof(ckpt_path)) {
            if (unlink(ckpt_path) != 0 && errno != ENOENT) {
                clog_warn(CLOG(LOGGER_ID), "failed to delete checkpoint file %s", ckpt_path);
            }
        }

        n = snprintf(ckpt_path, sizeof(ckpt_path), "%s.cv.ckpt", checkpoint_prefix);
        if (n > 0 && n < (int)sizeof(ckpt_path)) {
            if (unlink(ckpt_path) != 0 && errno != ENOENT) {
                clog_warn(CLOG(LOGGER_ID), "failed to delete checkpoint file %s", ckpt_path);
            }
...
void read_problem_multi(const char **classfiles, int nclasses)
{
    ...
    int *counts = (int *) malloc(sizeof(int) * (size_t) nclasses);
    int total = 0;
    for (int i = 0; i < nclasses; i++) {
        counts[i] = count_sequences(classfiles[i]);
        total += counts[i];
    }
    ...
    int offset = 0;
    for (int i = 0; i < nclasses; i++) {
        clog_info(CLOG(LOGGER_ID), "reading %d sequences from %s (label=%d)", counts[i], classfiles[i], i+1);
        read_fasta_file(classfiles[i], offset, i+1);
        offset += counts[i];
    }
